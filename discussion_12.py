...
def create_employee_table(cur, conn):
    cur.execute("CREATE TABLE IF NOT EXISTS employees (employee_id INTEGER PRIMARY KEY, first_name TEXT, last_name TEXT, hire_date TEXT, jod_id INT, salary INT)")
    conn.commit()
...
def add_employee(filename, cur, conn):
    ...
    for item in js_data:
        ...
        cur.execute("INSERT OR IGNORE INTO employees (employee_id, first_name, last_name, hire_date, job_id, salary) (? ? ? ? ? ?)", (employee_id, first_name, last_name, hire_date, job_id, salary))
    conn.commit()
...
def visualization_salary_data(cur, conn):
    cur.execute("SELECT employees.salary, Jobs.job_title FROM employee JOIN Jobs ON employees.job_id = Jobs.job_id")
